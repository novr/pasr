...
正本は D1（`absences`, `member_master`, `channel_notify_settings`）。
...
- domain パース、D1 repository / schema-check、absence-registration、absence-mention-parse、署名検証、event dedupe、Queue ack/retry、daily-notify、channel-config、ops-report
...
6. `npx wrangler deploy`（または Workers Builds: `main` push で自動 deploy）
...
- `/pasr-admin status` -> 直近 run 要約 + `db:` / `channel_notify_settings:` 状態
- `/pasr-admin run` -> daily notify 手動実行（即時 ACK 後 Queue 経由で非同期。ops レポートは投稿しない）
- `/pasr-admin channel-config empty on|off|default` -> 実行 CH の 0件時通知を上書き（同期応答。queue 不可）
- `/pasr-admin channel-config list` -> CH 別 0件時通知の上書き一覧（同期応答）
