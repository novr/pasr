...
- `vars.SLACK_PASR_USERS_USERGROUP_ID`（任意: `@pasr_users` 等の User Group **encoded ID**（`S...`）。handle `@pasr_users` は不可。未設定時は usergroup 操作をスキップ）

正本は D1（`absences`, `member_master`）。

### 6b) User Group 自動追加（任意）
- Slack で `@pasr_users`（任意名）を作成し、Group ID（`S...`）を `vars.SLACK_PASR_USERS_USERGROUP_ID` に設定
...
- App Home（`app_home_opened`, `tab=home`）:
  - 開くたびに本人の通知設定と今後の不在（最大5件）を D1 読取して `views.publish`
  - 登録・通知設定・不在一覧ボタン。Home 上の編集/削除成功時は Home を refresh
