...
- 前提:
  ...
  2. ターミナル A: `npm run dev`（`wrangler.jsonc` の `ai.remote: true` で Workers AI を利用）
  3. `.dev.vars` に `RUN_ENDPOINT_TOKEN` と `DEBUG_ENDPOINTS_ENABLED=true`（結合テスト実行時に自動読み込み）
...
- `app_mention`（チャンネル直下のみ）:
  ...
  - 本文付きメンション → Workers AI で日付・note を解釈 → ephemeral 確認 UI（登録 / キャンセル / Modal）→ 確認後 List 書き込み
  - AI 解釈失敗時は Modal ボタンへフォールバック
...
- `block_actions`: `pasr_register_open` / `pasr_mention_confirm` / `pasr_mention_cancel` / `pasr_absence_edit_open` / `pasr_absence_delete`
...
- 入口:
  ...
  - チャンネル直下の `@bot` メンション（スレッド内メンションは除外）
    ...
    - 本文付き（例: `@PASR 明日 通院`）→ Workers AI 解釈 → 確認 UI → 登録（通知先は `member_master` 既定）
