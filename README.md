...
2. 初回のみ Queue を作成: `npx wrangler queues create pasr-admin-tasks`
3. `npx wrangler deploy`
4. `/health` で readiness を確認
5. デプロイ出力に `env.ADMIN_TASK_QUEUE (pasr-admin-tasks)` と Consumer/Producer が表示されることを確認
6. `/run` を `POST` + トークン付きで 1 回実行し、ログに run summary が出ることを確認
...
| `slackLists.access.set` | admin への write 権限付与 |
