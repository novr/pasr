...
6. `npm run deploy`（`--keep-vars` 付き。または Workers Builds: Deploy command を `npm run deploy` に設定し `main` push で自動 deploy）
...
- `/pasr-admin status` -> 直近 run 要約 + `db:` / `channel_notify_settings:` 状態（`sent` は CH+DM 合計）
