...
| `views:write` | `views.open`（Modal）/ `views.publish`（App Home） |
...
**DM 自然文登録を有効にする追加設定（api.slack.com）**

1. OAuth & Permissions → Bot Token Scopes に `im:history` を追加
2. Event Subscriptions → Subscribe to bot events に `message.im` を追加（`app_mention` は維持）
3. App Home → Messages Tab を有効化し、メッセージタブからの送信を許可
4. **Install App → Reinstall to Workspace**

**App Home Tab を有効にする追加設定（api.slack.com）**

1. OAuth & Permissions → Bot Token Scopes に `views:write` を追加（未追加の場合）
2. Event Subscriptions → Subscribe to bot events に `app_home_opened` を追加
3. App Home → Home Tab を有効化
4. **Install App → Reinstall to Workspace**
