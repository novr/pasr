...
ログは JSON 形式。`event=daily_notify_done` の `run_id` / `errors` を確認する。`sent` は CH+DM 合計、内訳は `sent_channels` / `sent_dms`。土日の scheduled は通知せず ops レポートのみ（`skip_weekend_scheduled_notify`、`daily_notify_done.weekend_ops_only`）。
