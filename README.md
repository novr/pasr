...
正本は D1（`absences`, `member_master`）。Slack List からの初回取込は cutover 時の `POST /admin/import-lists` のみ。
...
- domain パース、D1 repository / import-gate / schema-check、absence-registration、absence-mention-parse、署名検証、event dedupe、Queue ack/retry、daily-notify

### 非カバー（単体テスト）

- 本番 Slack API、Workers ランタイム統合、cutover 後の import endpoint 実呼び出し
...
8. **D1 cutover（初回のみ）**: 下記 Runbook の import を実行

### D1 cutover Runbook（初回のみ）
1. deploy 後、register 試行 → 「データ移行中」ephemeral（import gate 確認）
2. `curl -X POST -H "Authorization: Bearer $RUN_ENDPOINT_TOKEN" $WORKER_URL/admin/import-lists`
3. レスポンス `processed` が absence **9** / member_master **12** であること
4. `npx wrangler d1 execute pasr-db --remote --command "SELECT COUNT(*) FROM absences"` → **9**
5. register / `/pasr-admin run` が成功すること
6. import 完了後、Slack App を manifest 更新版で再インストール（`lists:*` / `files:*` 削除後）
...
- `GET /health`
  - 認証不要
  - `200 {"ok":true}`
- `POST /admin/import-lists`
  - `Authorization: Bearer <RUN_ENDPOINT_TOKEN>` 必須
  - 空 D1 + 未 import 時のみ実行。完了後は `409`
  - 成功時 `200 {"ok":true,"absences":{...},"memberMaster":{...}}`
- `POST /run`
  - `Authorization: Bearer <RUN_ENDPOINT_TOKEN>` 必須
...
- `/pasr-admin status` -> 直近 run 要約 + `db:` / `import:` 状態
...
**cutover 前（import 用・一時）**: `lists:read`, `files:read` — import 完了後 manifest から削除
...
**import 時のみ（cutover 後不要）**: `slackLists.items.list`, `files.list`, `auth.test`
