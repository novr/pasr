...
#### OAuth ルート
- `GET /slack/oauth/start?user_id=U…` — state 発行後 Slack authorize へリダイレクト（設定・App Home から利用）
...
- `block_actions`: `pasr_register_open` / `pasr_home_settings_open` / `pasr_home_list_open` / `pasr_mention_confirm` / `pasr_mention_cancel` / `pasr_absence_edit_open` / `pasr_absence_delete` / `pasr_status_oauth_disconnect`
