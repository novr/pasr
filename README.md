...
- `/pasr update <itemId>` -> 行 ID 指定で不在編集 Modal（一覧に ID 表示）
...
- `block_actions`: `pasr_register_open` / `pasr_absence_edit_open` / `pasr_absence_delete`
- List 書き込みまで同期 ACK。登録通知・一覧削除の再描画は `waitUntil` で非同期
- バリデーション失敗時は `response_action: errors` を返す

## 不在一覧・編集（`/pasr list` / `/pasr update`）
- 本人の `end_date >= today`（JST）の不在のみ表示（最大25行）。各行に `itemId` を表示（`/pasr update <itemId>` 用）
