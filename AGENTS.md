...
- `/pasr calendar` は選んだ CH の `notify_channels` 一致のみ（非 admin 可）。D1 に残る予定分のみ。`from` は今日 JST 以降。表示は日付見出し + 期間中の各日に展開（レコード数と表示行数は一致しない場合あり）。結果は Bot DM にスレッド投稿（本人のみ・ページングなし）。再実行のたびに新スレッド
